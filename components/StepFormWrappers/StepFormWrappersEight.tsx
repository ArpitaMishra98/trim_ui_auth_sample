...
function StepFormWrappersEight({
  stepperButton,
  register,
  errors,
  handleSubmit,
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const isValidHeight =
      /^[0-9]{1,3}(,[0-9]{3})*(\.[0-9]{1,2})?$/.test(newValue) &&
      parseFloat(newValue) >= 0;

    if (isValidHeight) {
      setValue(newValue);
      // Store the valid value in localStorage
      localStorage.setItem("storedValue_kg", newValue);
    } else {
      // Clear the value and localStorage if the input is not valid
      setValue("");
      localStorage.removeItem("storedValue_kg");
    }
  };

  // useEffect to retrieve value from localStorage on component mount
  useEffect(() => {
    const storedValue_kg = localStorage.getItem("storedValue_kg");
    if (storedValue_kg) {
      setValue(storedValue_kg);
    }
  ...
  return (
    <StepFormWrappersStyled>
      <Box className="consult_wrap_gap shape_backs">
        <Box className="shape_backs_imgs step8 cmn_htFormbl" />
            ...
            <Box className="hetmsrd_wraps">
              ...
              <Box className="type_answer_form">
                ...
                >
                  ...
                  value === undefined ||
                  value.trim() === "" ? (
                    <p style={{ color: "red" }}>Weight (Kg) is required</p>
                  ) : (
                    ""
                  )}
                  <Grid item xs={12}>
...
export default StepFormWrappersEight;
